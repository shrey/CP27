...
void solve(){
    re n; re q;
    for(ll i = 1; i<=n; i++) re val[i];
    fo(n-1){
        ll x,y; re x; re y;
        gr[x].pb(y); gr[y].pb(x);
    }
    dfs(1,0);
    ll idx[n+1]; // index of 1 to n present in the dfs spreading
    for(ll i = 0; i<n; i++) idx[pre[i]] = i;
    for(ll i = 0; i<n; i++) a[i] = val[pre[i]];
    build(1,0,n-1);
    while(q--){
        ll t; re t;
        if(t == 1){
            ll s,x; re s; re x;
            update(1,0,n-1,idx[s],x);
        }else{
            ll x; re x;
            ll l = idx[x], r = idx[x] + ss[x] - 1;
